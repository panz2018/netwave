...
1. **工具/skill 有无新坑、命令变更、版本漂移？** → 回写本文档技能地图，
   或 [工具升级指南](../../knowledge/工具升级指南.md) 对应节（如 OpenSpec CLI 升版后同步版本号）。
...
| OpenSpec CLI v1.13.1 | ✅ 已装 | npm 全局 + 本仓库 `openspec/`；CLI 升级后需在每个已初始化项目里跑 `openspec update`（见 [工具升级指南·OpenSpec 节](../../knowledge/工具升级指南.md)） |
| pnpm 12.4.2 | ✅ 已装 | nvm（node v26.4.0）下经 corepack 管理（`corepack enable`）。注意：corepack shim 首次运行会下载对应版本二进制并**交互式确认**，无人值守终端会假死——根 `package.json` 钉死 `"packageManager": "pnpm@<版本>"` 后自动确定版本，或临时设 `COREPACK_ENABLE_DOWNLOAD_PROMPT=0` |
